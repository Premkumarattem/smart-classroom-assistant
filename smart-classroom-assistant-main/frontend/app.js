...
let teacherNotesExecutionId = "";
let teacherNotesSessionId = null;
let flashcardsExecutionId = "";
let lastFlashcardsData = [];
let quizExecutionId = "";
let lastQuizData = [];

// Shared renderer for every "Verify AI Content" result panel — see
// backend/evidence.py for what signature_valid/binding_valid actually
// check and don't check.
function renderVerifyResult(elId, result) {
  const el = document.getElementById(elId);
  el.classList.remove("hidden");
  el.className = `verify-result status-${(result.status || "").toLowerCase()}`;
  const ok = (b) => (b === true ? '<span class="verify-ok">✅ Valid</span>' : b === false ? '<span class="verify-fail">❌ Failed</span>' : '<span class="verify-ok">— n/a</span>');
  el.innerHTML = `
    <div class="verify-execid">Execution ID: ${result.execution_id || "unknown"}</div>
...
async function verifyByExecutionId(elId, executionId, content) {
  const el = document.getElementById(elId);
  el.classList.remove("hidden");
  el.className = "verify-result";
  el.innerHTML = "Verifying…";
  if (!executionId) {
    el.innerHTML = "No evidence was sealed for this content — nothing to verify.";
    return;
  }
  try {
...
document.getElementById("stopClassBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    teacherNotesExecutionId = data.notes_execution_id || "";
    teacherNotesSessionId = teacherSessionId;
    document.getElementById("verifyNotesResult").classList.add("hidden");
...
document.getElementById("verifyNotesBtn").addEventListener("click", async () => {
  const elId = "verifyNotesResult";
  const el = document.getElementById(elId);
  el.classList.remove("hidden");
  el.className = "verify-result";
  el.innerHTML = "Verifying…";
  if (!teacherNotesSessionId) {
    el.innerHTML = "No evidence was sealed for this class's notes — nothing to verify.";
    return;
  }
  try {
    // Deliberately server-side: re-reads whatever ai_notes currently sits
    // in the database for this class, rather than trusting what the
    // browser has on screen — so if someone edited the note directly in
    // the DB after class ended, this catches it.
    const res = await fetch(`${API_BASE}/api/verify/notes/${teacherNotesSessionId}`);
    if (!res.ok) throw new Error("verify failed");
    renderVerifyResult(elId, await res.json());
  } catch (err) {
    el.innerHTML = `Couldn't reach the backend at ${API_BASE}.`;
...
document.getElementById("flashcardsBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    const data = await res.json();
    container.innerHTML = "";
    lastFlashcardsData = data.flashcards || [];
    flashcardsExecutionId = data.execution_id || "";
    const verifyBtn = document.getElementById("verifyFlashcardsBtn");
    verifyBtn.classList.toggle("hidden", !flashcardsExecutionId);
    document.getElementById("verifyFlashcardsResult").classList.add("hidden");
    lastFlashcardsData.forEach((card) => {
      const div = document.createElement("div");
...
document.getElementById("verifyFlashcardsBtn").addEventListener("click", () => {
  verifyByExecutionId("verifyFlashcardsResult", flashcardsExecutionId, lastFlashcardsData);
});
...
document.getElementById("quizBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    const data = await res.json();
    lastQuizData = data.quiz || [];
    quizExecutionId = data.execution_id || "";
    const verifyBtn = document.getElementById("verifyQuizBtn");
    verifyBtn.classList.toggle("hidden", !quizExecutionId);
    document.getElementById("verifyQuizResult").classList.add("hidden");
    renderQuiz(lastQuizData);
  } catch (err) {
    container.innerHTML = `<p class="placeholder">Couldn't reach the backend at ${API_BASE}.</p>`;
  } finally {
    btn.disabled = false;
...
document.getElementById("verifyQuizBtn").addEventListener("click", () => {
  verifyByExecutionId("verifyQuizResult", quizExecutionId, lastQuizData);
});
