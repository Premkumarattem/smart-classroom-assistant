...
def init_db():
    ...
    conn.execute(
        f"""
        CREATE TABLE IF NOT EXISTS ai_evidence_receipts (
            id {PK_TYPE},
    ...
    for column_sql in (
        ...
        "ALTER TABLE class_sessions ADD COLUMN notes_execution_id TEXT",
    ):
        try:
...
def seal_and_store(artifact_type: str, source_text: str, output_text: str, session_id: Optional[int]) -> str:
    """
    Seal an AI-generated artifact with the evidence layer (see evidence.py)
    and persist the receipt so it can be independently re-verified later,
    even after the class/session itself is long over. Returns the
    execution_id the frontend uses to look the receipt back up. Never
    raises — a receipt failing to save should never block the AI feature
    itself from returning its result to the user.
    """
    try:
        provider = MODEL_PROVIDER
        model = ANTHROPIC_MODEL if provider == "anthropic" else GROQ_TEXT_MODEL
        receipt = evidence.seal_evidence(
        ...
        conn = get_db()
        conn.execute(
            "INSERT INTO ai_evidence_receipts (execution_id, session_id, artifact_type, receipt_json, created_at) VALUES (?, ?, ?, ?, ?)",
            (receipt["execution_id"], session_id, artifact_type, json.dumps(receipt), now_iso()),
        )
        conn.commit()
        conn.close()
...
def _finalize_class_stop(session_id: int) -> Optional[dict]:
    ...
    notes_execution_id = ""
    if transcript.strip():
        notes_execution_id = seal_and_store("smart_notes", transcript, ai_notes, session_id)

    conn.execute(
        "UPDATE class_sessions SET end_time = ?, ai_notes = ?, status = 'ended', recording_path = ?, mic_muted = 0, camera_off = 0, notes_execution_id = ? WHERE id = ?",
        (now_iso(), ai_notes, final_recording_path, notes_execution_id, session_id),
    )
    conn.commit()
    conn.close()
    return {"ok": True, "ai_notes": ai_notes, "recording_path": final_recording_path, "notes_execution_id": notes_execution_id}
...
def generate_flashcards(req: FlashcardsRequest):
    ...
    try:
        ...
        # Seal the canonical (sorted-key) JSON, not the raw LLM text, so
        # verification later can re-canonicalize whatever JSON the client
        # sends back and get a byte-for-byte match regardless of how it
        # was re-serialized along the way.
        canonical = json.dumps(cards, sort_keys=True, separators=(",", ":"))
        execution_id = seal_and_store("flashcards", req.transcript, canonical, req.session_id)
        return {"flashcards": cards, "execution_id": execution_id}
    except Exception as e:
...
def generate_quiz(req: QuizRequest):
    ...
    try:
        ...
        canonical = json.dumps(quiz, sort_keys=True, separators=(",", ":"))
        execution_id = seal_and_store("quiz", req.transcript, canonical, req.session_id)
        return {"quiz": quiz, "execution_id": execution_id}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


# ---------------------------------------------------------------
# Evidence verification — "Verify AI Content" button on the frontend.
# See evidence.py for what this actually checks and doesn't. Two ways
# to verify:
#   1. GET  /api/verify/notes/{session_id}  — re-checks the *current*
#      ai_notes stored server-side for that class against its receipt.
#      This is the one that catches someone editing the DB directly.
#   2. POST /api/verify                     — for artifacts that are
#      never stored server-side (quiz, flashcards): the client sends
#      back whatever content it's currently displaying, and this
#      checks it against the sealed receipt.
# ---------------------------------------------------------------
def _load_receipt(execution_id: str) -> Optional[dict]:
    conn = get_db()
    row = conn.execute(
        "SELECT receipt_json FROM ai_evidence_receipts WHERE execution_id = ?", (execution_id,)
    ).fetchone()
    conn.close()
    if not row:
        return None
...
@app.get("/api/verify/notes/{session_id}")
def verify_notes(session_id: int):
    conn = get_db()
    row = conn.execute(
        "SELECT ai_notes, notes_execution_id FROM class_sessions WHERE id = ?", (session_id,)
    ).fetchone()
    conn.close()
    if not row or not row["notes_execution_id"]:
        raise HTTPException(status_code=404, detail="No sealed evidence exists for this class's notes.")
    receipt = _load_receipt(row["notes_execution_id"])
    if not receipt:
        raise HTTPException(status_code=404, detail="Receipt not found.")
    result = evidence.verify_evidence(receipt, current_output_text=row["ai_notes"] or "")
    return result


class VerifyContentRequest(BaseModel):
    execution_id: str
    content: object  # the JSON the client currently has (quiz array, flashcards array, or raw string)


@app.post("/api/verify")
def verify_content(req: VerifyContentRequest):
    receipt = _load_receipt(req.execution_id)
    if not receipt:
        raise HTTPException(status_code=404, detail="Unknown execution_id — no receipt was sealed under this id.")
    if isinstance(req.content, str):
        canonical = req.content
    else:
        canonical = json.dumps(req.content, sort_keys=True, separators=(",", ":"))
    result = evidence.verify_evidence(receipt, current_output_text=canonical)
    return result
