...
## Verifiable AI Content — tamper-evident receipts

Every AI-generated artifact — smart notes, flashcards, and quiz — is now
sealed with a cryptographic receipt the moment it's created, and a
**"🔒 Verify AI Content"** button lets anyone check it afterward.

**How it works, precisely:** when notes/flashcards/quiz are generated,
the backend hashes the source transcript and the AI's output (SHA-256),
bundles that with the model/provider/timestamp, and signs the bundle
with an Ed25519 private key that lives only on the server
(`backend/evidence.py`, keypair auto-generated on first run into
`backend/evidence_keys/`, gitignored). That signed bundle is the
"receipt." Clicking Verify re-checks two independent things: the
signature is still valid (the receipt itself wasn't altered), and the
content's hash still matches what was sealed (nobody edited the
artifact afterward). For smart notes specifically, verification
re-reads the live database rather than trusting what's on screen — so
editing a note directly in the DB and clicking Verify again visibly
flips the result to **TAMPERED**. That's the actual demo-worthy moment,
not the green checkmark by itself.

**What this honestly does and doesn't prove:** it proves a specific
hash was sealed at a specific time and hasn't changed since — the same
scope any receipt/evidence SDK in this space actually has. It does
**not** prove the AI's summary was factually correct, or that the
transcript itself was accurate. Say that plainly if a judge asks.

**Why this is a self-built module and not a wired-in third-party SDK:**
this was originally scoped around a sponsor SDK ("CooL") described as
doing exactly this — verifiable evidence for AI decisions with offline
verification and tamper detection. We were never able to get its actual
docs, package name, or API in hand. Rather than guess at a third-party
API and ship code that imports a package that might not exist (or
might be an unrelated same-named upload — this space is currently full
of very similarly-pitched "AI decision receipt" SDKs), we implemented
the real mechanics ourselves: genuine Ed25519 signing and SHA-256
hashing, not a mock. `evidence.py`'s `seal_evidence()` /
`verify_evidence()` functions were deliberately kept close to the
"hash in, receipt out" / "receipt in, verdict out" shape these SDKs
